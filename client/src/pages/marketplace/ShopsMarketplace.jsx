...
import {
    ...
    Ticket, Flame, Percent, Zap, BookOpen, Shirt,
    Sparkles, Package, Laptop
} from 'lucide-react';
...
const VoucherCard = ({ title, code, discount }) => (
    <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-md border border-dashed border-indigo-300 p-4 rounded-2xl flex items-center justify-between group hover:border-indigo-500 transition-all shadow-sm text-left">
        <div className="flex items-center gap-4">
            ...
            <div>
                ...
                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-tight">Code: <span className="text-indigo-500">{code}</span></p>
            </div>
        </div>
        <div className="text-right">
            <div className="text-lg font-black text-indigo-500 leading-none">{discount}</div>
    ...
    >
        {shop.isNew && (
            <div className="absolute top-4 right-4 z-20">
                <div className="bg-indigo-500 text-white px-3 py-1 rounded-lg text-[10px] font-black shadow-lg flex items-center gap-1.5">
                    <Flame size={12} /> NEW
                </div>
            </div>
        )}
        ...
        <div className="relative z-10 flex flex-col items-center gap-4">
            <div className="h-20 w-20 rounded-[1.5rem] bg-gradient-to-br from-indigo-400 via-purple-400 to-pink-500 p-0.5 shadow-lg group-hover:scale-110 transition-transform duration-500">
                <div className="h-full w-full rounded-[1.4rem] bg-white flex items-center justify-center overflow-hidden">
                    {shop.image ? (
                        <img src={shop.image} alt={shop.name} className="w-full h-full object-cover" />
                    ) : (
                        <span className="text-3xl font-black bg-clip-text text-transparent bg-gradient-to-br from-indigo-500 to-purple-500">
                            {shop.name.charAt(0)}
                        </span>
                    )}
                </div>
            ...
            <div className="space-y-1">
                <h3 className="text-xl font-black text-gray-900 dark:text-white leading-tight group-hover:text-indigo-500 transition-colors">
                    {shop.name}
                </h3>
            ...
            <Button className="w-full mt-2 rounded-2xl bg-indigo-500 hover:bg-indigo-600 opacity-100 transition-all duration-300">
                Visit Shop
            </Button>
        </div>
    </div>
...
const ProductCard = ({ product, onClick, onAddToCart }) => (
    <div className="group relative bg-white dark:bg-gray-800 backdrop-blur-xl border border-white/60 dark:border-gray-700/60 rounded-2xl overflow-hidden shadow-[0_2px_10px_rgb(0,0,0,0.02)] hover:shadow-lg hover:-translate-y-1 transition-all duration-300 text-left">
        {/* Image Section */}
        <div className="h-40 w-full bg-indigo-50 dark:bg-gray-700 p-4 flex items-center justify-center relative overflow-hidden">
            {product.image ? (
                <img
                    src={product.image}
                    alt={product.title}
        ...
        <div className="p-4 space-y-3">
            ...
            <h3 
                className="font-bold text-gray-900 dark:text-white text-sm line-clamp-1 group-hover:text-indigo-500 transition-colors cursor-pointer"
                onClick={onClick}
            >
                {product.title}
            </h3>
            ...
            <button
                ...
                className="w-full py-2.5 bg-indigo-500 hover:bg-indigo-600 disabled:bg-gray-300 text-white text-xs font-bold rounded-xl transition-all flex items-center justify-center gap-2"
            >
                <ShoppingBag size={14} />
...
const ShopsMarketplace = () => {
    ...
    const offers = [
        { title: "Tech Week", desc: "10% off on all tech accessories.", gradient: "from-blue-400 to-indigo-600", icon: Zap },
        { title: "Stationery Sale", desc: "Buy 1 Get 1 on all notebooks.", gradient: "from-emerald-400 to-teal-600", icon: BookOpen },
        { title: "Merch Drop", desc: "Limited edition hoodies available.", gradient: "from-purple-400 to-fuchsia-600", icon: Shirt }
    ];

    const vouchers = [
        { title: "Tech Credit", code: "TECH10", discount: "10% OFF" },
        { title: "Store Bonus", code: "SYNC25", discount: "৳280 OFF" }
    ];
    ...
    return (
        <div className="relative min-h-screen p-3 md:p-5 space-y-6 font-sans">
            ...
            <div className="fixed inset-0 -z-30 pointer-events-none">
                <div className="absolute top-0 left-[-100px] w-[600px] h-[800px] bg-gradient-to-br from-indigo-100/50 via-purple-100/50 to-transparent rounded-full mix-blend-multiply blur-[80px]"></div>
                <div className="absolute top-[-10%] right-[-10%] w-[600px] h-[600px] bg-gradient-to-bl from-pink-100/50 to-indigo-100/50 rounded-full mix-blend-multiply blur-[80px]"></div>
            </div>
            ...
            <div className="max-w-7xl mx-auto space-y-6">
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <button
                            ...
                            className="p-2.5 rounded-2xl bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 shadow-sm hover:border-indigo-500 hover:text-indigo-500 transition-all group"
                        >
                            <ArrowLeft size={20} className="group-hover:-translate-x-1 transition-transform" />
                        </button>
                        <div>
                            <div className="flex items-center gap-2">
                                <h1 className={`text-2xl font-black ${selectedShop ? 'text-gray-400 hover:text-indigo-500 cursor-pointer' : 'text-gray-900 dark:text-white'}`}
                                ...
                                {selectedShop && (
                                    <>
                                        ...
                                        <h1 className="text-2xl font-black text-indigo-500">{selectedShop.name}</h1>
                                    </>
                                )}
                            </div>
                            <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1">
                                {selectedShop ? 'Browse and order products' : 'Startups & retail stores on campus'}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                        {selectedShop && (
                            <Button
                                ...
                                className="rounded-2xl px-4 border-indigo-200 text-indigo-500 hover:bg-indigo-50"
                            >
                                <MessageCircle size={18} className="mr-2" />
                                Chat
                            </Button>
                        )}
                        <div className="relative cursor-pointer" onClick={() => navigate('/marketplace/shops/cart')}>
                            <Button variant="outline" size="icon" className="rounded-xl shadow-sm bg-white hover:text-indigo-500 p-3">
                                <ShoppingBag size={24} />
                            </Button>
                            {cartCount > 0 && (
                                <div className="absolute -top-1 -right-1 h-5 w-5 bg-indigo-500 text-white text-[10px] font-black rounded-full flex items-center justify-center shadow-lg border-2 border-white">
                                    {cartCount}
                                </div>
                            )}
                        </div>
                ...
                <div className="bg-white/60 dark:bg-gray-800/60 backdrop-blur-xl rounded-[2rem] p-4 border border-white shadow-sm">
                    <div className="relative flex-1 group">
                        <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400 group-focus-within:text-indigo-500 transition-all" />
                        <input
                            type="text"
                            placeholder={selectedShop ? `Search items in ${selectedShop.name}...` : "Search campus shops..."}
                            value={searchQuery}
                            onChange={(e) => setSearchQuery(e.target.value)}
                            className="w-full pl-12 pr-4 py-3.5 rounded-2xl bg-gray-50/50 dark:bg-gray-900/50 border-2 border-transparent focus:border-indigo-200 focus:bg-white focus:outline-none transition-all font-medium"
                        />
                    </div>
                </div>
                    ...
                    <div className="space-y-10">
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className="text-xl font-black text-gray-900">Campus Shops</h3>
                    ...
                    <div className="space-y-6">
                            ...
                            {filteredProducts.length > 0 ? (
                                filteredProducts.map(product => (
                                    <ProductCard
                                        key={product.id}
                                        product={product}
                                        onClick={() => navigate(`/marketplace/shops/${product.id}`)}
                        ...
                        {vendorCartItems.length > 0 && (
                            <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50">
                                <div 
                                    onClick={() => navigate('/marketplace/shops/cart')}
                                    className="bg-indigo-500 hover:bg-indigo-600 text-white px-8 py-4 rounded-2xl shadow-2xl shadow-indigo-500/30 flex items-center gap-6 cursor-pointer transition-all hover:-translate-y-1"
                                >
                                    <div className="flex items-center gap-3">
                                        <ShoppingBag size={24} />
...
export default ShopsMarketplace;
