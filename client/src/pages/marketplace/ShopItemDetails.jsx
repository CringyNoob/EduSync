...
import {
    ArrowLeft, Heart, MessageCircle, Share2, Shield, MapPin,
    CheckCircle, ShoppingBag, Minus, Plus, Store, Package
} from 'lucide-react';
        ...
        return (
            <div className="flex items-center justify-center min-h-screen bg-white">
                <div className="text-center max-w-md p-8">
                    ...
                    <Button onClick={() => navigate('/marketplace/shops')} className="w-full py-4 rounded-2xl bg-indigo-500 hover:bg-indigo-600">
                        Back to Shops
                    </Button>
                </div>
            </div>
    ...
    return (
        <div className="relative min-h-screen p-6 font-sans pb-32">
            ...
            <div className="fixed inset-0 -z-50 pointer-events-none">
                <div className="absolute top-[20%] right-[10%] w-[400px] h-[400px] bg-indigo-100/30 rounded-full blur-[80px]"></div>
                <div className="absolute top-[10%] left-[10%] w-[300px] h-[300px] bg-purple-100/30 rounded-full blur-[60px]"></div>
            </div>
            ...
            <div className="max-w-6xl mx-auto px-4 grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16">
                ...
                <div className="space-y-4">
                    <div className="aspect-square w-full rounded-[3rem] shadow-2xl overflow-hidden border border-white/60 relative group bg-indigo-50 flex items-center justify-center">
                        {product.image ? (
                            <img
                                src={product.image}
                                alt={product.title}
                            ...
                            <Package size={120} className="text-indigo-200" />
                        )}
                        {!product.is_available && (
                            <div className="absolute inset-0 bg-gray-900/60 flex items-center justify-center">
                                <span className="bg-red-500 text-white px-6 py-3 rounded-2xl text-lg font-black">Out of Stock</span>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="flex flex-col pt-2 text-left">
                    <div className="mb-8 border-b border-gray-100 pb-8">
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                            ...
                            <div className="text-left sm:text-right flex flex-col items-start sm:items-end min-w-fit">
                                <div className="text-3xl md:text-4xl font-black text-indigo-500 tracking-tight">৳{product.price}</div>
                    ...
                    {vendor && (
                        <div 
                            ...
                            className="bg-white/60 backdrop-blur-xl rounded-[2.5rem] p-6 border border-white shadow-sm mb-6 flex items-center gap-5 group hover:border-indigo-200 transition-all cursor-pointer"
                        >
                            <div className="h-14 w-14 rounded-2xl bg-gradient-to-br from-indigo-400 to-purple-500 flex items-center justify-center text-white font-bold text-xl shadow-lg shadow-indigo-100 transition-transform group-hover:scale-110 overflow-hidden">
                                {vendor.logo ? (
                    ...
                    <div className="bg-white/60 backdrop-blur-xl rounded-[2rem] p-6 border border-white shadow-sm mb-6">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                {currentQuantity > 0 && (
                                    <p className="text-[10px] text-indigo-500 font-bold mt-1">{currentQuantity} already in cart</p>
                                )}
                            </div>
                            <div className="flex items-center gap-4">
                                ...
                                <button
                                    ...
                                    className="h-12 w-12 rounded-xl bg-indigo-500 hover:bg-indigo-600 text-white flex items-center justify-center transition-colors"
                                >
                                    <Plus size={20} />
                                </button>
                            </div>
                        ...
                        <div className="mt-4 pt-4 border-t border-gray-100 flex items-center justify-between">
                            ...
                            <span className="text-2xl font-black text-indigo-500">৳{(parseFloat(product.price || 0) * quantity).toFixed(0)}</span>
                        </div>
                    </div>
                    ...
                    <div className="mt-auto flex gap-4">
                        ...
                        {vendor && vendor.owner_id && vendor.owner_id !== user?.id && (
                            <ChatWithVendorButton
                                ...
                                variant="outline"
                                className="flex-1 py-6 text-lg border-indigo-200 bg-white text-indigo-500 hover:bg-indigo-50 rounded-[1.5rem]"
                            >
